		...
		private IEnumerable<Direction> ScanNeighbours(Terrain[][] myArea, Inhabitant inhabitant)
		{
			foreach (var direction in Forest.Directions.Keys)
			{
				...
				if (myArea[newPoint.Y][newPoint.X].Name == "Unknown")
				{
					...
					yield return Forest.Directions[direction];
					...
					else
					{
						...
						yield return Forest.Directions[direction.Negative()];
					}
				}
			}
		}
					...
					if (i == front.Count - 1)
					{
						if (newFront.Count != 0)
							foreach (var move in GetPath(parents, front[i], newFront[0]))
								yield return Forest.Directions[move];
					}
					else
